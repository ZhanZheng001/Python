'''
输入一个由N个大小写字母组成的字符串,按照ASCII码值从小到大进行排序
    查找字符串中第K个最小ASCII码值的字母(k>=1),输出该字母所在字符串中的位置索引(字符串的第一个位置索引为0)
...
while True:
    try:
        str1 = 'fAdDAkBbBq'.strip()
        k = 4
        list1 = sorted(list(str1))
        if k < len(str1):
            tmp = list1[k-1]
            print(str1.index(tmp))
            break
        else:
            print(str1.index(list1[-1]))
            break
    except:
        break