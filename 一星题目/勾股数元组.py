...
'''
def f1(x,y,z):#判断勾股数
    return True if x*x+y*y==z*z else False

def f2(x,y,z):#判断两两是否有公约数
    for i in range(2,z+1):#质数从2开始
        if x%i==0 and y%i==0:return False
        elif y%i==0 and z%i==0:return False
        elif x%i==0 and z%i==0:return False
        else:continue
    return True
