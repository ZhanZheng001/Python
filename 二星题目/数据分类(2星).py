...
while 1:
    try:
        c, b, *nums = list(map(int, "3 4 256 257 258 259 260 261 262 263 264 265".split()))
        dp = {}
        for a in nums:
            tmp = hex(a)[2:].zfill(8) # 转16进制
            sum1 = 0
            for i in range(0, len(tmp), 2): # 拆分成4段后相加
                sum1 += int(tmp[i: i+2], 16)
            t = sum1 % b # 取模
            if t < c:
                if t in dp: # 分类
                    dp[t] += 1
